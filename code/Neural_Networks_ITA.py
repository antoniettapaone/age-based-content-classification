...
def predict_with_bad_words_check_lstm(model, texts, titles, tokenizer, bad_words):
...
def predict_without_bad_words_check_lstm(model, texts, titles, tokenizer):
    sequences = tokenizer.texts_to_sequences([preprocess_text(text) for text in texts])
    X = pad_sequences(sequences, maxlen=100)
    predictions = model.predict(X)
...
if len(texts) == 0 or len(labels) == 0:
    ...
    def create_lstm_model():
        model = Sequential([
            ...
            LSTM(64, return_sequences=False),  # LSTM layer con 64 unità
    ...
    for train_index, test_index in kfold.split(X, y):
        ...
        y_pred_with, detected_bad_words_list = predict_with_bad_words_check_lstm(model, texts_test, titles_test, tokenizer, bad_words)
        ...
        y_pred_without = predict_without_bad_words_check_lstm(model, texts_test, titles_test, tokenizer)
    ...
    print("Report di classificazione LSTM con controllo delle badwords (k-fold cross-validation):\n", report_with_badwords)
    ...
    print("Report di classificazione LSTM senza controllo delle badwords (k-fold cross-validation):\n", report_without_badwords)
...
# Definizione dell'architettura LSTM con dropout
def create_lstm_model():
    model = Sequential([
        Embedding(input_dim=10000, output_dim=64),  # Embedding per rappresentare le parole in vettori
        LSTM(64, dropout=0.2, recurrent_dropout=0.2),  # LSTM con dropout e recurrent dropout
        Dense(3, activation='softmax')  # Strato finale per la classificazione
    ])
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    return model
