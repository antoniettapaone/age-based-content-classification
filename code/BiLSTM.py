...
get_ipython().system('pip install numpy pandas scikit-learn tensorflow nltk tqdm')
...
def predict_with_bad_words_check_bilstm(model, texts, titles, tokenizer, bad_words):
...
def predict_without_bad_words_check_bilstm(model, texts, titles, tokenizer):
    sequences = tokenizer.texts_to_sequences([preprocess_text(text) for text in texts])
    X = pad_sequences(sequences, maxlen=100)
    predictions = model.predict(X)
...
if len(texts) == 0 or len(labels) == 0:
    ...
    # Definizione dell'architettura BiLSTM
    def create_bilstm_model():
        model = Sequential([
            Embedding(input_dim=10000, output_dim=64),
            Bidirectional(LSTM(64, return_sequences=False)),  # LSTM bidirezionale
            Dense(64, activation='relu'),  # Strato denso
            Dense(3, activation='softmax')  # Strato finale per la classificazione
        ])
    ...
    for train_index, test_index in kfold.split(X, y):
        ...
        # Addestramento del modello BiLSTM
        model = create_bilstm_model()

        # Addestramento del modello BiLSTM con tqdm per visualizzare la barra di avanzamento
        num_epochs = 10
        batch_size = 32
        ...
        y_pred_with, detected_bad_words_list = predict_with_bad_words_check_bilstm(model, texts_test, titles_test, tokenizer, bad_words)
        ...
        y_pred_without = predict_without_bad_words_check_bilstm(model, texts_test, titles_test, tokenizer)
    ...
    print("Report di classificazione BiLSTM con controllo delle badwords (k-fold cross-validation):\n", report_with_badwords)
    ...
    print("Report di classificazione BiLSTM senza controllo delle badwords (k-fold cross-validation):\n", report_without_badwords)
...
# Verifica delle stopwords in inglese
english_stopwords = set(stopwords.words('english'))
print("Esempi di stopwords inglesi:", list(english_stopwords)[:10])
...
if len(texts) == 0 or len(labels) == 0:
    ...
    # Definizione dell'architettura BiLSTM con dropout
    def create_bilstm_model():
        model = Sequential([
            Embedding(input_dim=10000, output_dim=64),
            Bidirectional(LSTM(64, dropout=0.2, recurrent_dropout=0.2)),  # BiLSTM con dropout e recurrent dropout
            Dense(3, activation='softmax')  # Strato finale per la classificazione
        ])
        model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        return model
...
from tensorflow.keras.preprocessing.sequence import pad_sequences
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Embedding, Conv1D, GlobalMaxPooling1D, LSTM, Dense, Dropout, Reshape
from tensorflow.keras.callbacks import LambdaCallback
from tqdm import tqdm
from nltk.tokenize import word_tokenize
from nltk.corpus import stopwords
...
# Funzione per la predizione con controllo delle "badwords"
def predict_with_bad_words_check_combined(model, texts, titles, tokenizer, bad_words):
    sequences = tokenizer.texts_to_sequences([preprocess_text(text) for text in texts])
    X = pad_sequences(sequences, maxlen=100)
    predictions = model.predict(X)
    adjusted_predictions = []
...
# Funzione per la predizione senza controllo delle "badwords"
def predict_without_bad_words_check_combined(model, texts, titles, tokenizer):
    sequences = tokenizer.texts_to_sequences([preprocess_text(text) for text in texts])
    X = pad_sequences(sequences, maxlen=100)
    predictions = model.predict(X)
    predicted_labels = [np.argmax(prediction) for prediction in predictions]
...
if len(texts) == 0 or len(labels) == 0:
    ...
    # Definizione dell'architettura combinata CNN + LSTM
    def create_combined_model():
        model = Sequential([
            Embedding(input_dim=10000, output_dim=64),
            Conv1D(filters=64, kernel_size=3, activation='relu'),  # Layer CNN per estrarre le caratteristiche locali
            GlobalMaxPooling1D(),  # Pooling globale
            Reshape((1, 64)),  # Aggiunge una dimensione da (batch_size, 64) a (batch_size, 1, 64)
            LSTM(64, dropout=0.2, recurrent_dropout=0.2),  # Layer LSTM per gestire le sequenze
            Dense(64, activation='relu'),
            Dropout(0.5),  # Dropout per prevenire overfitting
            Dense(3, activation='softmax')  # Strato di output per la classificazione
        ])
        model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
        return model
    ...
    for train_index, test_index in kfold.split(X, y):
        ...
        # Crea il modello combinato CNN + LSTM
        model = create_combined_model()

        # Addestramento del modello combinato con tqdm per visualizzare la barra di avanzamento
        num_epochs = 10
        batch_size = 32
        with tqdm(total=num_epochs) as pbar:
            model.fit(X_train, y_train, epochs=num_epochs, batch_size=batch_size, verbose=0, callbacks=[LambdaCallback(on_epoch_end=lambda _, __: pbar.update(1))])
        ...
        # Predizione sui dati di test con controllo delle "badwords"
        y_pred_with, detected_bad_words_list = predict_with_bad_words_check_combined(model, texts_test, titles_test, tokenizer, bad_words)
        all_y_true_with.extend(y_test)
        all_y_pred_with.extend(y_pred_with)
        all_detected_bad_words.extend(detected_bad_words_list)

        # Predizione sui dati di test senza controllo delle "badwords"
        y_pred_without = predict_without_bad_words_check_combined(model, texts_test, titles_test, tokenizer)
        all_y_true_without.extend(y_test)
        all_y_pred_without.extend(y_pred_without)

    # Report di classificazione dettagliato con controllo delle "badwords"
    report_with_badwords = classification_report(all_y_true_with, all_y_pred_with, target_names=['0_bambini', '1_ragazzi', '2_adulti'])
    print("Report di classificazione combinato CNN + LSTM con controllo delle badwords (k-fold cross-validation):\n", report_with_badwords)

    # Report di classificazione senza controllo delle "badwords"
    report_without_badwords = classification_report(all_y_true_without, all_y_pred_without, target_names=['0_bambini', '1_ragazzi', '2_adulti'])
    print("Report di classificazione combinato CNN + LSTM senza controllo delle badwords (k-fold cross-validation):\n", report_without_badwords)
